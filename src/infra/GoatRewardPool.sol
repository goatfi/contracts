...
contract GoatRewardPool is ERC20, Ownable {
    ...
    /// @notice An existing reward has been added to be distributed
    ...
    /// @notice The duration is too short or too long to be set
    ...
    /// @param _amount Amount of GOA to stake
    function stake(uint256 _amount) external update(msg.sender) {
        _stake(msg.sender, _amount);
    }

    /// @notice Stake GOA tokens with a permit
    /// @dev An equal number of receipt tokens will be minted to the caller
    ...
    /// @notice Withdraw GOA tokens
    /// @dev Burns an equal number of receipt tokens from the caller
    /// @param _amount Amount of GOA to withdraw
    function withdraw(uint256 _amount) external update(msg.sender) {
        _withdraw(_amount);
    }

    /// @notice Withdraw all of the caller's GOA tokens and claim rewards
    ...
    function removeReward(address _reward, address _recipient) external onlyOwner {
        ...
        IERC20(_reward).safeTransfer(_recipient, rewardBal);
    ...
    /// @dev Stake GOA tokens and mint the caller receipt tokens
    ...
    /// @dev Withdraw GOA tokens and burn an equal number of receipt tokens from the caller
    /// @param _amount Amount of GOA to withdraw
    ...
    /// @dev Calculate the reward amount per GOA token
    /// @param _reward Address of the reward
    /// @return rewardPerToken Reward amount per GOA token
