...
/// @notice Strategy managing the rewards from the GOA reward pool
contract StakedGOAStrategy is StratFeeManagerInitializable {
    using SafeERC20 for IERC20;

    /// @notice GOA token address
    address public want;
    ...
    /// @notice Reward pool for GOA rewards
    ...
    /// @param protocolFees Amount of native sent to the protocol fee recipient
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IGoatRewardPool(rewardPool).stake(wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IGoatRewardPool(rewardPool).withdraw(_amount - wantBal);
    ...
    function _harvest(address _callFeeRecipient) internal whenNotPaused {
        IGoatRewardPool(rewardPool).getReward();
        _swapToNative();
    ...
    function _swapToNative() internal {
        for (uint i; i < rewards.length; ++i) {
            ...
            if (rewardBal > 0) IGoatSwapper(unirouter).swap(reward, native, rewardBal);
        }
    }
    ...
    function _swapToWant() internal {
        ...
        if (nativeBal > 0) IGoatSwapper(unirouter).swap(native, want, nativeBal);
    }
    ...
    function balanceOfWant() public view returns (uint256 balanceHeld) {
        balanceHeld = IERC20(want).balanceOf(address(this));
    }
    ...
    function balanceOfPool() public view returns (uint256 balanceInvested) {
        balanceInvested = IERC20(rewardPool).balanceOf(address(this));
    }
    ...
    function rewardsAvailable() public view returns (uint256 unclaimedReward) {
        unclaimedReward = IGoatRewardPool(rewardPool).earned(address(this), native);
    }
    ...
    function retireStrat() external {
        ...
        IGoatRewardPool(rewardPool).withdraw(balanceOfPool());
    ...
    function setReward(address _reward) external onlyOwner {
        ...
        IERC20(_reward).forceApprove(unirouter, type(uint).max);
    ...
    function removeReward(address _reward) external onlyManager {
        ...
        rewards.pop();
        IERC20(_reward).forceApprove(unirouter, 0);
        emit RemoveReward(_reward);
    }

    /// @dev Give out allowances to third party contracts
    function _giveAllowances() internal {
        IERC20(want).forceApprove(rewardPool, type(uint).max);
    ...
    function _removeAllowances() internal {
        IERC20(want).forceApprove(rewardPool, 0);
