...
contract GoatVaultDeploymentSiloTest is Test {
    ...
    address want = AssetsArbitrum.USDCe; // USDCe
    ...
    function test_CanCompleteTestCycle() public {
        // Get want
        uint256 amountToDeposit = 100000 * 10e6;
        ...
        uint256 collateralBalance = IERC20(collateral).balanceOf(address(strategy));
        uint256 balanceOfPool = strategy.balanceOf();

        strategy.harvest();

        assertGt(IERC20(native).balanceOf(protocolFeeRecipient), feeBatchBalance, "FeeBatch");
        assertGt(IERC20(native).balanceOf(strategist), strategistBalance, "Strategist");
        assertGt(IERC20(collateral).balanceOf(address(strategy)), collateralBalance, "Collateral");
        assertGt(strategy.balanceOf(), balanceOfPool, "Balance of Pool");
